...
function loginSubmit(ev) {
    ...
    params.append('username', usernameEl.value);
    ...
    fetch('login', {
        ...
        showMsg('loginMsg', d.success ? 'Uspjesna prijava!' : ' ' + d.message, d.success);
        if (d.success) { setTimeout(function() { location.href = 'index.html'; }, 600); }
    }).catch(function() {
        showMsg('loginMsg', ' Greska u komunikaciji sa serverom.', false);
    });
}
...
function registerSubmit(ev) {
    ...
    if (username.length < 3) {
        showMsg('registerMsg', 'Korisnicko ime mora imati najmanje 3 znaka.', false);
        return;
    }
    if (password.length < 4) {
        showMsg('registerMsg', 'Lozinka mora imati najmanje 4 znaka.', false);
        return;
    }
        ...
        .then(function(d) {
            ...
            if (d.success) {
                setTimeout(function() {
                    ...
                    showMsg('loginMsg', 'Registracija uspjesna! Unesite podatke za prijavu.', true);
                }, 900);
            }
        }).catch(function() {
            showMsg('registerMsg', ' Greska u komunikaciji sa serverom.', false);
        });
}
...
function openPlayer(id) {
    fetch('playerStats?id=' + encodeURIComponent(id))
        .then(function(r) { return r.json(); })
        .then(function(p) {
            if (!p.success) { alert(p.message || 'Greska'); return; }
                ...
                '<div class="player-modal-grid">' +
                  ...
                  '<div>' +
                    ...
                    '<div class="stats-grid">' +
                      ...
                      '<div class="stat-box"><strong>' + p.yellow_cards + '</strong>Zuti</div>' +
            ...
            alert('Ne mogu ucitati statistiku. Provjeri bazu i servlet.');
        });
}
...
function openTrophy(id) {
    fetch('trophy?id=' + encodeURIComponent(id))
        .then(function(r) { return r.json(); })
        .then(function(t) {
            if (!t.success) { alert(t.message || 'Greska'); return; }
            ...
            alert('Ne mogu ucitati trofej. Provjeri bazu i servlet.');
        });
}
